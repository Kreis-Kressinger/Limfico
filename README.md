...
Limfico also comes bundled together with Limkey, a command line utility that can generate randomized keys for Limfico. It's installation and usage guide are also included in this document. Limkey is equally lightweight and should generate even large keys quickly.
...
If you just want to compile one of the two programs, then just enter: `make Limfico` for Limfico or `make Limkey` for Limkey.
...
- `-i (name of the input file)` Input can have any extension. If the input file doesn't exist, the program won't work.
- `-o (name of the output file)` Output can have any extension. If the output file doesn't exist, the program will create one for you.
- `-k (name of the key file)` Key file can have any file extension. I am using `.bin` here because it's the extension of keys generated by Limkey.
- `-c` *(completely optional.)* Skip the confirmation step.
...
Limkey will then generate a key with your given key length, with a maximum of 18,446,744,073,709,551,614. The key will be named `key(random letters).bin`. Limkey doesn't take any other parameters other than `{key length}`.
